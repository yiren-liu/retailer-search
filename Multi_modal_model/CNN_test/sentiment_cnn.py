...
def load_data(data_source):
    ...
    if data_source == "keras_data_set":
        ...
        x1 = x[shuffle_indices]
        y = y[shuffle_indices]
        train_len = int(len(y) * 0.9)
        x_train = x1[:train_len]
        y_train = y[:train_len]
        x_test = x1[train_len:]
        y_test = y[train_len:]
    ...
    return x_train, y_train, x_test, y_test, vocabulary_inv,x_unknown


# Data Preparation
print("Load data...")
x_train, y_train, x_test, y_test, vocabulary_inv,x_unknown = load_data(data_source)
...
words=[vocabulary_inv[i[0]] for i in x_unknown]
result=[(words[i],y[i]>0.5)for i in range(len(x_unknown))]
...
plt.title("Training Loss and Accuracy")#Loss and
plt.xlabel("Epoch #")
plt.ylabel("Loss/Accuracy")#Loss/
...
#     if i[1][0]==True:
#         print(i[0])
#         predict_entity.append(i[0])
# print(len(x_unknown))
# import pymongo
# cl=pymongo.MongoClient('localhost')
# col=cl.news.data
# col.insert_one({'predict_company_entity':predict_entity})