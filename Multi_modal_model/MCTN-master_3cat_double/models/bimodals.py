...
class E2E_MCTN_Model(BaseModel):
  ...
  def _create_model(self):
    ...
    #second model
    print("Creating TRANSLATION SEQ2SEQ model ...")
    inputs_2, encoded_seq_2, decoded_seq_2, cycled_decoded_seq_2 = \
        mctn_model(output_dim=output_dim,
                   hidden_dim=self.configs['translation']['hidden_dim'],
                   output_length=output_length,
                   input_dim=input_dim,
    ...
    regression_score = \
      create_regression_model(
        ...
        input=concatenate([encoded_seq,encoded_seq_2],-2),
        l2_factor=self.configs['regression']['l2_factor'],input_hidden=self.configs['translation']['hidden_dim'],
      dropout=0.5)
    ...
    end2end_model = Model(inputs=[inputs,inputs_2],
    ...
    end2end_model.compile(loss=[self.configs['translation']['loss_type'],
  ...
  def train(self,
    ...
    return  self.model.fit(x=[self.input_train,self.output_train],
                   y=[self.output_train,
                      self.input_train,
                      self.feats_dict['train_labels']
                      ],
                   epochs=n_epochs,
                   validation_data=[[self.input_test,self.output_test],[self.output_test,self.input_test,
                                                                        self.feats_dict['test_labels']]],#self.input_test,
  ...
  def predict(self):
    ...
    preds = self.model.predict([self.input_test,self.output_test])
