...
def data_stats():
    result_file = '../data/result_retailer.csv'
    label_file = 'labels_3_cat_big.npy'
    ...
    #
    #         labels.append(line[3])
    labels = np.load('../data/labels_3_cat_big.npy')
    print('sample number: ', len(labels))
    dic = collections.Counter(list(labels))
...
def draw_word_num_pic():
    ...
    plt.show()
    print(len(des_num))
    print(pd.array(des_num).description())


def draw_cross_test_pic():
    df = pd.read_csv('../data/cross_test/last.csv', encoding='utf-8')
    x = []
    for i in range(10):
        x.append(df['c'].values[10 * i:10 * (i + 1)])

    plt.plot(np.array(x).mean(0))
    plt.show()
    print(np.array(x).mean(0))
    pass
